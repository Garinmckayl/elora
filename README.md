...
|-------|------------|-----|
| **Mobile** | Expo / React Native (TypeScript) | Cross-platform, fast iteration, Expo Go for testing |
...
| **Gemini 2.0 Flash** | Text agent, sub-agents, auto-memorise, proactive evaluator, face recognition |
...
- **Live call mode** — Continuous bidi audio; natural interruptions in both directions
- **Immersive call UI** — Full-screen camera feed, floating controls, live captions, AI scanning indicator, audio waveform
- **Hold-to-talk** — Single utterance mode
- **Voice activity detection** — Model-level, no silence padding

### Vision & Face Recognition
- **Camera capture** — Point and ask; single-shot JPEG -> Gemini multimodal
...
- **Auto-memorise** — Background task extracts and stores key facts after every exchange
- **Memory compaction** — Gemini Flash merges/deduplicates raw facts into a categorised user profile
- **Session memory** — Post-call summarisation; last 3 summaries injected into next session
- **Research loop** — LoopAgent with self-verification: searches, evaluates quality, retries up to 3 times
- **Emotional intelligence** — Samantha-inspired persona; warm, direct, remembers context
...
│   ├── elora_agent/
│   │   └── agent.py              # Multi-agent hierarchy, system prompt, 30+ tool defs
...
│   │   ├── session_memory.py     # Post-call summarisation
...
│   ├── demo-script.md            # Demo choreography + Devpost copy
│   └── progress.md               # Build log (9 days)
└── README.md
```
